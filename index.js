...
var debugLogContent = "";
function viewMyLogs() {
	var v = '<span ng-app="SalesforceSimplifiedApp" id="SalesforceSimplified1"><span ng-controller="MyViewGridCtrl"><span><input class="btn" title="By Salesforce Simplified" value="My Logs" style="background: #1796bf; color: white;" ng-click="queryForDebugLogs()" type="button" ng-strict-di/></span><span><debugloggrid></debugloggrid></span></span></span>';
		...
		$.ajax({
	         ...
	         success: function(result) {
	          if(result && result.records && result.records.length>0){
		          setCookie('NamespacePrefix', result.records[0].NamespacePrefix+'__', 2);
		          setCookie('isNamespacePrefixAvailable', true, 2);
		          return true;
	      	}else{
	      		return false;
	      	 }
